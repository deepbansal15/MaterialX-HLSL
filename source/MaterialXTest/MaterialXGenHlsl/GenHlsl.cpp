...
#include <MaterialXTest/MaterialXGenHlsl/GenHlsl.h>
...
TEST_CASE("GenShader: HLSL Syntax Check", "[genhlsl]")
{
    mx::TypeSystemPtr ts = mx::TypeSystem::create();
    mx::SyntaxPtr syntax = mx::HlslSyntax::create(ts);
...
TEST_CASE("GenShader: HLSL Implementation Check", "[genhlsl]")
{
...
TEST_CASE("GenShader: HLSL Unique Names", "[genhlsl]")
{
    mx::GenContext context(mx::HlslShaderGenerator::create());
...
TEST_CASE("GenShader: HLSL Light Shaders", "[genhlsl]")
{
...
TEST_CASE("GenShader: HLSL Performance Test", "[genhlsl]")
{
    mx::GenContext context(mx::HlslShaderGenerator::create());
...
static void generateHlslCode()
{
    ...
    // Create the HLSL shader generator
    mx::ShaderGeneratorPtr generator = mx::HlslShaderGenerator::create();

    const mx::FilePath logPath("genhlsl_generate_test.txt");
    HlslShaderGeneratorTester tester(generator, testRootPaths, searchPath, logPath, false);

    // Test with resource binding context
    mx::HlslResourceBindingContextPtr hlslResourceBinding = mx::HlslResourceBindingContext::create();
    tester.addUserData(mx::HW::USER_DATA_BINDING_CONTEXT, hlslResourceBinding);
...
TEST_CASE("GenShader: HLSL Shader Generation", "[genhlsl]")
{
    generateHlslCode();
}

TEST_CASE("GenShader: HLSL Shader Generation with Resource Binding", "[genhlsl]")
{
    // Test with separate uniform and sampler binding locations
    mx::FileSearchPath searchPath = mx::getDefaultDataSearchPath();
    ...
    const mx::FilePath logPath("genhlsl_resource_binding_test.txt");
    HlslShaderGeneratorTester tester(generator, testRootPaths, searchPath, logPath, false);

    mx::HlslResourceBindingContextPtr hlslResourceBinding = mx::HlslResourceBindingContext::create(0, 100);
    tester.addUserData(mx::HW::USER_DATA_BINDING_CONTEXT, hlslResourceBinding);
